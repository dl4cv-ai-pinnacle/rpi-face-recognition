"""Simple cross-frame face tracking helpers."""
...
class SimpleFaceTracker:
    ...
    def update(
        ...
        *,
            ...
            if track_idx in matched_track_indices:
                ...
                next_tracks.append(
                    self._update_matched_track(
                        ...
                        det_kps=None if kps is None else np.asarray(kps[det_idx], dtype=np.float32),
                    )
                )
                continue
        ...
        ranked_tracks.sort(key=lambda track: track.track_id)
        self._tracks = ranked_tracks
    ...
    def _match_detections(self, boxes: Float32Array) -> list[tuple[int, int]]:
        ...
        for track_idx, track in enumerate(self._tracks):
            for det_idx, det_box in enumerate(boxes):
                iou = _box_iou(track.box[:4], np.asarray(det_box[:4], dtype=np.float32))
