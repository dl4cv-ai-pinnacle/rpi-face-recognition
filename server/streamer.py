...
class CameraStreamer:
    ...
    def __init__(self, runtime: LiveRuntime, config: AppConfig) -> None:
        ...
        self._raw_jpeg_bytes: bytes | None = None
        self._error: str | None = None
        self._stop_event = threading.Event()
        self._thread: threading.Thread | None = None
        self._frame_interval = 1.0 / config.live.target_fps if config.live.target_fps > 0 else 0.0
    ...
    def wait_for_frame(self, last_seen: int, timeout: float = 5.0) -> StreamSnapshot:
        with self._condition:
            ...
            return StreamSnapshot(
                ...
                raw_jpeg_bytes=self._raw_jpeg_bytes,
                error=self._error,
            )
    ...
    def _capture_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if not ok_raw:
                    raise RuntimeError("OpenCV failed to encode a raw JPEG frame")
                ...
                self._runtime.record_frame_metrics(frame_result=frame_result, loop_ms=loop_ms)
                self._publish(encoded.tobytes(), raw_jpeg_bytes=encoded_raw.tobytes())
    ...
    def _publish(
        ...
        *,
        ...
        with self._condition:
            ...
            self._raw_jpeg_bytes = raw_jpeg_bytes
            self._error = error
            self._condition.notify_all()
